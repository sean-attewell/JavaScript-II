// A local community center is holding a fund rasising 5k fun run and has invited 50 small businesses to make a small donation on their behalf for some much needed updates to 
// their facilities.  Each business has assigned a representative to attend the event along with a small donation.
...
// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and 
// log the result
// let allCaps = [];
...
// **WORKING**
function adder(acc, currentPerson) {
    return acc + currentPerson.donation;
  }

const ticketPriceTotal = runners.reduce(adder, 0)
...
// Problem 1 - return all company names, but make each start with the "The" and end with "company" 

let companyNames = []

runners.forEach(element => companyNames.push(`The ${element.company_name} company`));

console.log(companyNames)


// Problem 2 - count how many people made a donation under 100 for public shaming

let under100 = runners.filter(element => element.donation < 100)

console.log(under100)
console.log(under100.length)


// Problem 3 - pump the numbers. Produce a new array where you artificially raising all donations by 100.

let dodgyFigures = runners.map(element => element.donation += 100);

console.log(dodgyFigures);
